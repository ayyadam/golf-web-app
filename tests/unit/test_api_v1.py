...
class TestListTeeTimes:
    ...
    def test_returns_available_tee_times(self, client, tee_time):
    ...
    def test_includes_slots_remaining(self, client, tee_time):
        resp = client.get('/api/v1/tee-times')
        assert resp.status_code == 200
        body = resp.get_json()
    ...
    def test_filter_by_date(self, client, db, future_date):
        tt = TeeTime(
        ...
        _db.session.add(tt)
        _db.session.commit()
        resp = client.get(f'/api/v1/tee-times?date={future_date.isoformat()}')
        assert resp.status_code == 200
        body = resp.get_json()
        assert all(t['date'] == future_date.isoformat() for t in body)

    def test_excludes_unavailable(self, client, db, future_date):
        tt = TeeTime(
...
class TestGetTeeTime:
    ...
    def test_returns_single_tee_time(self, client, tee_time):
    ...
    def test_unknown_id_returns_404(self, client, db):
        resp = client.get('/api/v1/tee-times/9999999')
...
class TestListCompetitions:
    ...
    def test_returns_upcoming_competitions(self, client, competition):
    ...
    def test_excludes_past_competitions(self, client, db, past_date):
